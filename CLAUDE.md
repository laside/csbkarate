...
│   ├── rebuild_database.sql ← reconstruction complète (teardown + migrations 0001→0020 + seed saison V2.0 + comptes de test), un seul Run — DEV uniquement
...
- Exception : sur `galerie.html`, seule la **structure** (quelle photo dans quelle section/stage) était enregistrée en ligne. Désormais, les **nouvelles images** sont uploadées directement via l'interface admin dans le bucket Supabase `galerie`. (Les anciennes images GitHub sont conservées pour la rétrocompatibilité). Depuis l'optim stockage, ces uploads passent par **`CSBFiles.prepare`** (`assets/js/files.js`) : validation type/taille + compression (redimension ≤ 1600 px + WebP) avant envoi. ⚠️ La **suppression** d'une photo dans l'admin ne retire que l'URL de la structure JSON, **pas** l'objet du bucket (fichier conservé — une même image peut être partagée entre sections ; nettoyage manuel si besoin).
...
- [ ] **Pièces justificatives — optimisation stockage (plan gratuit Supabase : 1 Go Storage, 500 Mo DB, 5 Go egress/mois)** — l'essentiel **livré** sur `feature/optim-pieces-jointes` (module `assets/js/files.js`, `window.CSBFiles`) :
  - [x] **Validation à l'upload** ✅ — `CSBFiles.validate` (type jpg/png/webp/pdf + taille) appelé dans `adherent.js` (`uploadPiece` + ajout), `inscription.js` (avant le signUp), `galerie.js` et `news.js`. Option `imageOnly` sur galerie/news → **refuse les PDF** (images seulement).
  - [x] **Compression image côté client** ✅ — `CSBFiles.compressImage` (canvas → **WebP**, redimension ≤ 1200 px pièces / ≤ 1600 px galerie-news, qualité 0.8 ; fallback à l'original si aucun gain ou si `toBlob` WebP non supporté).
  - [x] **Fuite Storage corrigée** ✅ — suppression adhérent **simple** (`adherent.js`) ET **multiple** (`membres.js`) purgent les objets du bucket `dossiers` (`photo_path` + `documents_files`) via `storage.remove([...])` (best-effort, non bloquant).
  - [ ] **Garde-fou serveur** (RESTE) : configurer sur le bucket `dossiers` un `file_size_limit` et `allowed_mime_types` (Dashboard > Storage > bucket, ou SQL `storage.update_bucket`) — l'UI se contourne, pas la config bucket.
  - [ ] **Rétention / purge** (RESTE) : purge des pièces des dossiers `Annulé` et des saisons archivées (RGPD : données de mineurs) + suivi périodique de la conso (Dashboard > Settings > Usage).
