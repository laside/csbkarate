...
**TODO restants :**
- [ ] **Exécuter dans Supabase (SQL Editor) la migration `0016_saisons_cours.sql`** (refonte saisons/cours). ⚠️ **Bloquante** pour l'affichage dynamique de l'accueil/club et le calcul d'inscription : crée les tables `saisons` + `cours` (RLS lecture publique / écriture bureau) et reprend la saison 2026-2027. Tant qu'elle n'a pas tourné, `index.html`/`club.html` gardent leur fallback statique et l'inscription retombe sur `DEFAULT_CONFIG` (`tarifs.js`). **Édition désormais dans l'Espace Bureau** (`membres.html`, onglet « Saisons & Cours ») — l'ancien mode admin d'`index.html` (édition saison) a été supprimé. L'ancienne table `saison` (JSONB, 0005) n'est plus lue (laissée comme trace).
